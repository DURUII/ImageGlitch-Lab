...
export default function FlashPhotoPage({ params }: { params: { locale: string } }) {
  // Title flash
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      on = !on
      document.title = on ? `${baseTitle} [ON]` : baseTitle
    }, 1200)
  ...
  useEffect(() => {
    ...
    const restoreFromShared = async () => {
      try {
        const img = await loadImage(sharedImage)
        if (cancelled) return
      ...
      if (!cancelled) {
        startUploadTransition(sharedImage)
      }
    }

    void restoreFromShared()
    return () => {
      cancelled = true
    }
